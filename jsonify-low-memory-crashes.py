...
def process_json(s):
    ...
    for i in range(180,-1,-1):
        ...
        if start_date not in s:
            lowmem_count = get_total_from_json(start_date, url + '&system_memory_use_percentage=>%3D90')
            total_count = get_total_from_json(start_date, url)
            lowmem_percent = int(float(lowmem_count)/float(total_count)*100)
            s += '{"date":"' + start_date + '","count":' + str(lowmem_count) + ', "percent":' + str(lowmem_percent) + '}'
