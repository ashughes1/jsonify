...
def process_json(url):
    ...
    response = urllib2.urlopen(url)
    json_get = response.read()
    pos_adapters = json_get.find('adapter_driver_version')
    ...
    for item in items:
        ...
        i = item.split(',')
        if len(i) == 2:
            driver = i[1].strip()[9:-1]
            count = i[0].strip()[9:]
            if count != '':
                data[driver] = float(count)/float(total)*100
    return data
...
url_root = "https://crash-stats.mozilla.com/api/SuperSearch/?product=Firefox&_facets=adapter_driver_version&_facets_size=25&date=>%3D2015-05-18"
...
for vendor in vendors:
    ...
    for d in drivers:
        ...
        string += '{"driver":"' + d + '","release":' + str(values[0]) + ',"beta":' + str(values[1]) + ',"aurora":' + str(values[2]) + ',"nightly":' + str(values[3]) + '}'
