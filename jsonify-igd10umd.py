...
def process_json(string, channel):
    today = date.today()
    ...
    for i in range(180,-1,-1):
        ...
        if start_date not in string:
            query = 'https://crash-stats.mozilla.com/api/SuperSearch/?date=%3E%3D' + start_date + '&date=%3C' + end_date + '&signature=~igd10umd&release_channel=' + channel
            string += get_total_from_json(start_date, query)
            if i >= 0: string += ','
        progress = int((float(180) - float(i)) / float(180) * 100)
        sys.stdout.write("Processing " + channel + ": %d%% \r" % (progress) )
        sys.stdout.flush()
    return string
...
if len(sys.argv) <= 1:
    ...
    for i in range(0, len(string)):
        if i == 0:
            f.write('[' + process_json(string[i].strip('[]'), 'nightly') + '],')
        elif i == 1:
            f.write('[' + process_json(string[i].strip('[]'), 'aurora') + '],')
        elif i == 2:
            f.write('[' + process_json(string[i].strip('[]'), 'beta') + ']')
        elif i == 3:
            f.write('[' + process_json(string[i].strip('[]'), 'release') + ']')
