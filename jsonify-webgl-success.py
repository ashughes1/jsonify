...
def process_json(string):
    ...
    for key in result.keys():
        for platform in platforms:
            ...
            url = ""
            if key == '?':
                url = 'https://crash-stats.mozilla.com/api/SuperSearch/?app_notes=~WebGL%3F&date=%3E%3D{:s}&platform_pretty_version=~{:s}&_histogram.date=_cardinality.install_time&_histogram_interval=1d'.format(str(start_date),platform)
            elif key == '+':
                url = 'https://crash-stats.mozilla.com/api/SuperSearch/?app_notes=~WebGL%2B&date=%3E%3D{:s}&platform_pretty_version=~{:s}&_histogram.date=_cardinality.install_time&_histogram_interval=1d'.format(str(start_date),platform)
            elif key == '-':
                url = 'https://crash-stats.mozilla.com/api/SuperSearch/?app_notes=~WebGL-&date=%3E%3D{:s}&platform_pretty_version=~{:s}&_histogram.date=_cardinality.install_time&_histogram_interval=1d'.format(str(start_date),platform)
