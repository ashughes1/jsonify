...
def process_json(s):
    ...
    for i in range(180,-1,-1):
        ...
        url = 'https://crash-stats.mozilla.com/api/SuperSearch/?date=%3E%3D' + start_date + '&date=%3C' + end_date
        if start_date not in s:
            gce_count = get_total_from_json(start_date, url + '&graphics_critical_error=!__null__')
            total_count = get_total_from_json(start_date, url)
            gce_percent = int(float(gce_count)/float(total_count)*100)
            s += '{"date":"' + start_date + '","count":' + str(gce_count) + ', "percent":' + str(gce_percent) + '}'
