...
def process_json(string):
    ...
    for d in data['facets']['histogram_date']:
        ...
        for hardware in d['facets']['android_hardware']:
            percent = float(hardware['count'])/float(total)*100
            if hardware['term'] == 'qcom':
                string += ',"qcom":{:.2f}'.format(percent)
            elif hardware['term'] == 'smdk4x12':
                string += ',"smdx4x12":{:.2f}'.format(percent)
            elif hardware['term'] == 'universal5420':
                string += ',"universal5420":{:.2f}'.format(percent)
            elif hardware['term'] == 'espresso10':
                string += ',"espresso10":{:.2f}'.format(percent)
            elif hardware['term'] == 'redhookbay':
                string += ',"redhookbay":{:.2f}'.format(percent)
        string += '}' 
