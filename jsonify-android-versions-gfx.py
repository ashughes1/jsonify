...
def process_json(string):
    ...
    data = get_json('https://crash-stats.mozilla.com/api/SuperSearch/?product=FennecAndroid&release_channel=release&release_channel=beta&signature=~gfx&signature=~layers&signature=~canvas&date=%3E%3D' + str(today) + '&_facets=android_version&_facets_size=5')
    versions = ''
    counts = {}
    for d in data['facets']['android_version']:
        counts[d['term'][0:2]] = 0
        versions += '&android_version=~{0}'.format(d['term'][0:2])
    data = get_json('https://crash-stats.mozilla.com/api/SuperSearch/?product=FennecAndroid&release_channel=release&release_channel=beta&signature=~gfx&signature=~layers&signature=~canvas' + versions + '&date=%3E%3D' + str(today) + '&_histogram.date=android_version&_histogram_interval.date=1d')
