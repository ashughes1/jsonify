...
def process_json(s):
    ...
    for i in range(180,-1,-1):
        ...
        url = 'https://crash-stats.mozilla.com/api/SuperSearch/?date=%3E%3D' + start_date + '&date=%3C' + end_date + '&signature=~gfx&signature=~layers&adapter_vendor_id=0x1002'
        if start_date not in s:
            total = get_total_from_json(start_date, url)
            count = get_total_from_json(start_date, url + '&adapter_driver_version=%2415')
