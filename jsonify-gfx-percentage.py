...
def process_json():
    ...
    for i in range(180,-1,-1):
        ...
        if start_date not in string:
            gfx_count = get_total_from_json(start_date, 'https://crash-stats.mozilla.com/api/SuperSearch/?date=%3E%3D' + start_date + '&date=%3C' + end_date + '&signature=~gfx&signature=~layers')
            total_count = get_total_from_json(start_date, 'https://crash-stats.mozilla.com/api/SuperSearch/?date=%3E%3D' + start_date + '&date=%3C' + end_date)
            gfx_percent = int(float(gfx_count)/float(total_count)*100)
            string += '{"date":"' + start_date + '","total":' + str(gfx_percent) + '}'
            if i > 0:
                string += ','
