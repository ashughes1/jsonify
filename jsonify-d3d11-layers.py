...
def process_json(string):
    ...
    for key in result.keys():
        for platform in platforms:
            ...
            url = ""
            if key == '?':
                url = 'https://crash-stats.mozilla.com/api/SuperSearch/?app_notes=~D3D11 Layers%3F&date=%3E%3D{:s}&platform_pretty_version=~{:s}&_histogram.date=_cardinality.install_time&_histogram_interval=1d'.format(str(start_date),platform)
            elif key == '+':
                url = 'https://crash-stats.mozilla.com/api/SuperSearch/?app_notes=~D3D11 Layers%2B&date=%3E%3D{:s}&platform_pretty_version=~{:s}&_histogram.date=_cardinality.install_time&_histogram_interval=1d'.format(str(start_date),platform)
            elif key == '-':
                url = 'https://crash-stats.mozilla.com/api/SuperSearch/?app_notes=~D3D11 Layers-&date=%3E%3D{:s}&platform_pretty_version=~{:s}&_histogram.date=_cardinality.install_time&_histogram_interval=1d'.format(str(start_date),platform)
