...
def process_json(s):
    ...
    for i in range(180,-1,-1):
        ...
        if start_date not in s:
            shutdown_count = get_total_from_json(start_date, url + '&shutdown_progress=shutdown&shutdown_progress=xpcom')
            total_count = get_total_from_json(start_date, url)
            shutdown_percent = int(float(shutdown_count)/float(total_count)*100)
            s += '{"date":"' + start_date + '","count":' + str(shutdown_count) + ',"percent":' + str(shutdown_percent) + '}'
